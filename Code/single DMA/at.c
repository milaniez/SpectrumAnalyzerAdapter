...
inline void dma_init (void)
{
    ...
    DMA.CH0.REPCNT = 0x01;
    DMA.CH0.SRCADDR = ((unsigned long)(&ADCA.CH0.RES));
    DMA.CH0.DESTADDR = ((unsigned long)(&(fft_adc.adc_data[fft_N])));
...
inline void rearrange_adc_data (void)
{
    unsigned int i;
    for (i = 0; i < fft_N; i++)
        fft_adc.adc_data[i] = fft_adc.adc_data[fft_N + anagram(i)];
}
...
//    for (i = 0; i < 256; i++)
//    {
//        dac_inputs_min[i] = 31*i/32;
//        dac_inputs_max[i] = 31*i/32 + 8;
//    }
//
//    while (1);
    while (1)
    {
        ...
        for (i = 1; i < ((fft_N >> 1) - 1); i++)
        {
            dac_inputs_max[((fft_N >> 1) - 1) - i] = ucmax3(fft_adc.fft_abs[i],(fft_adc.fft_abs[i] + fft_adc.fft_abs[i + 1])*0.5,
                (fft_adc.fft_abs[i] + fft_adc.fft_abs[i - 1])*0.5);
            dac_inputs_min[((fft_N >> 1) - 1) - i] = ucmin3(fft_adc.fft_abs[i],(fft_adc.fft_abs[i] + fft_adc.fft_abs[i + 1])*0.5,
                (fft_adc.fft_abs[i] + fft_adc.fft_abs[i - 1])*0.5);
        }
